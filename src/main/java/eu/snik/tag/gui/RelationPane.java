...
public class RelationPane extends VBox {
	...
	RelationPane(State state, Runnable createRestorePoint) {
		...
		Button addButton = new Button("Verbindung hinzufügen");
		{
			...
			addButton.setOnAction(
				e -> {
					if (subjectBox.getValue() == null || objectBox.getValue() == null || predicateBox.getValue() == null) {
						return;
					}
	...
	public void setSubject(Clazz first) {
		subjectBox.setValue(first);
	}

	public void setObject(Clazz first) {
		objectBox.setValue(first);
	}
}
